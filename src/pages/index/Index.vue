  ...
  <page-wraper :use-wx-ad="false" :use-reward-fab="true">
    <view class="page">
      <view class="page__hd">
        ...
        <view class="page__desc">113</view>
      </view>
      <view class="page__bd">
        <block v-for="(item, index) in list" :key="index">
          <view class="category">
            <view :id="item.id" class="category__hd" @click="kindToggle(item.id)">
              <image class="kind-list__img" :src="item.icon"></image>
              <view class="category__title">{{ item.name }}</view>
              <view :class="['category__arrow', openState[item.id] ? 'category__arrow--open' : '']"></view>
            </view>
            <view v-if="openState[item.id]" class="category__bd">
              <view v-for="(page, j) in item.pages" :key="j" class="page-item" @click="handleClick(`/subPages/${page.id}/Index`)">
...
// 创建一个状态来跟踪每个分类的打开状态，默认全部展开
const openState = ref<Record<string, boolean>>(Object.fromEntries(list.value.map((item) => [item.id, true])))
